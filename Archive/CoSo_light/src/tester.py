...
def get_random_complex_dom(n_domains):
    ...
    if type > 2:
        ...
        if type == 3:
            dom = f"inter({dom_left},{dom_right})"
        else:
            dom = f"union({dom_left},{dom_right})"
    return dom
...
def generate_problem(n_domains,u_size,is_sequence,struct_size,spec, choice_constraints,counting_constraints):
    ...
    if counting_constraints:
        ...
        for i in range(1,n_constr):
            ...
            problem += f"count(s,{dom} {op} {n}).\n"
    return problem

def domf2minizinc(df,n):
    if df.functor == "not":
        aux_d, name, n = domf2minizinc(df.args[0],n)
        descr = f"\nset of int: df{n+1}= universe diff {name};\n"
        return aux_d + descr, f"df{n+1}", n+1
    elif df.functor == "inter":
        aux_dl, namel, n = domf2minizinc(df.args[0],n)
        aux_dr, namer, m = domf2minizinc(df.args[1],n)
        descr = f"\nset of int: df{m+1}= {namel} intersect {namer};\n"
        return aux_dl+ aux_dr + descr, f"df{m+1}", m+1
    elif df.functor == "union":
        aux_dl, namel, n = domf2minizinc(df.args[0],n)
        aux_dr, namer, m = domf2minizinc(df.args[1],n)
...
def problem2minizinc(problem):
    ...
    if sequence:
        minizinc += f"array[1..n] of var {problem.universe}: sequence;\n"
        if problem.structure.spec:
            minizinc += "constraint alldifferent(sequence);\n"
    elif subset:
        minizinc += f"var set of {problem.universe}: sub;\n"
    ...
    for cof in problem.count_formulas:
        ...
        if cof.op == "\=":
            op = "!="
        aux_doms, dom_f, n  = domf2minizinc(cof.dformula.formula, n)
        for l in aux_doms.split('\n'):
            if l not in minizinc:
                minizinc += l + "\n"
        minizinc += f"constraint sum({range})(bool2int({elem} in {dom_f})) {op} {cof._val};\n"
        ...
        for s in size_struct:
            for c in cases:
                if d>s:
                    ...
                    print(problem)
                    Path("tests/constrained").mkdir(parents=True, exist_ok=True)
                    filename = f"tests/constrained/{name}_{d}_{s}.test"
                    file = open(filename, "w")
                    file.write(problem)
                    file.close()
...
if __name__ == '__main__':
    ...
    #     min = 'minizinc' in args.compare
    #     test_folder(args.test_folder, ap, min)
    else:
        parser.print_help()
